...
namespace JNIAccessor {
    ...
    void JavaEnv::Setup() {
		...
		jmethodID dispose =            FindMethod(frame, "dispose", "()V");
		jmethodID pack =               FindMethod(jframe, "pack", "()V");
		jmethodID setDefaultCloseOp =  FindMethod(jframe, "setDefaultCloseOperation", "(I)V");
		//jmethodID setLocationRelTo =   FindMethod(jframe, "setLocationRelativeTo", "(Ljava.awt.Component;)V");
		...
		if (theWindow == NULL) {
			...
			env->CallVoidMethod(ourFrame, setDefaultCloseOp, 3/*JFrame.EXIT_ON_CLOSE*/);
			//env->CallVoidMethod(ourFrame, setLocationRelTo, NULL);
			...
			if (size > 0) {
				...
				env->CallVoidMethod(theWindow, dispose);
			...
			env->DeleteLocalRef(AWTLock);
			env->DeleteLocalRef(theWindow);
			env->DeleteLocalRef(comps);
		}
    }
